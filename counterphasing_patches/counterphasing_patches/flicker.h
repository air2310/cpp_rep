...
public:
	...
	Matrix flickrates(int ftrial, std::vector<double> Hz, int nHz, flicktype flick2use, double alphalevel, int refreshrate) {
		Matrix FLICKS(nHz*2, Row(ftrial));

		for (int FF = 0; FF < nHz; FF++) {
			switch (flick2use) {
			...
			case flicktype::smooth:
				...
				FLICKS[FF+nHz] = countsmoothflick(ftrial, Hz[FF], alphalevel, refreshrate);
				break;
			}
...
private:
	...
	std::vector<double> countsmoothflick(double ftrial, double Hz, double alphalevel, int refreshrate) {

		std::vector<double> wave = COUNTSIN(Hz, alphalevel, ftrial, refreshrate);
	...
	std::vector<double> COUNTSIN(double Hz, double alphalevel, int ftrial, int  refreshrate) {
		std::vector<double> wave(ftrial);
		...
		double a = alphalevel / 2;
		for (double x = 0; x < ftrial; x++) {
			wave[x] = a*sin(2 * pi / frames*x-pi) + a;
		}
