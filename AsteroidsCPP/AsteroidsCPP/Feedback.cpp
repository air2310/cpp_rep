...
void Feedback(HDC hdc, HWND hWnd, int points, int nWidth, int nShip, int nHeight, int xxtra, int border, std::vector<Row>& ColorCond, int Trial, std::vector<int> Order)
{
	...
	gr.DrawString(L"Total", -1, &font, PointF((nWidth / 2) - 300, nHeight / 2 + 125), &whiteBrush);
	...
	HBITMAP bmp = CreateCompatibleBitmap(hdc, 150, 350);
	HANDLE hOld = SelectObject(memDC, bmp);

	SelectObject(memDC, bmp);

	HFONT hFont1 = (HFONT)SelectObject(hdc, &font);
...
#define WHITE     0xFFFFFF
	...
	if (points < -99 & points > -1000) {
		...
		TextOut(memDC, 0, 15, str, sizeof(str));
		TextOut(memDC, 0, 115, str, sizeof(str));
		TextOut(memDC, 0, 215, str, sizeof(str));
	}
	...
	if ((points < 100) & (points > 9)) {
		char str[3];
		memset(str, '\0', sizeof(str));
		sprintf_s(str, "%d", points); TextOut(memDC, 0, 15, str, sizeof(str));
		TextOut(memDC, 0, 15, str, sizeof(str));
		TextOut(memDC, 0, 115, str, sizeof(str));
		TextOut(memDC, 0, 215, str, sizeof(str));
	}
	if ((points < 1000) & (points > 99)) {
		char str[4];
		memset(str, '\0', sizeof(str));
		sprintf_s(str, "%d", points);
		TextOut(memDC, 0, 15, str, sizeof(str));
	...
	BitBlt(hdc, (nWidth / 2)+nShip*2, nHeight / 2 - 50 - 35.5, nShip*3, 330, memDC, 0, 0, SRCCOPY);
	...
	gr.FillRectangle(&whiteBrush, (nWidth / 2) - 300, nHeight / 2 + 110, 550, border / 2);
