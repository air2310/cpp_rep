...
class cueperiod {
public:
	sf::RectangleShape rendercue(int width, int height, int numsquares, int Trial, int xxtra) {
		...
		sf::Vector2f cueloc = sf::Vector2f(i * tileSize.x + 10 + xxtra, j * tileSize.y + 10);
	...
	sf::CircleShape rendercuedot(int width, int height, int numsquares, int Trial, int xxtra) {
		...
		sf::Vector2f cueloc = sf::Vector2f(i * tileSize.x + tileSize.x/2 - 5 + xxtra, j * tileSize.y + tileSize.y /2 -5);

		sf::CircleShape cueshape(10);
		cueshape.setOutlineColor(sf::Color(0, 0, 0, 0));
		cueshape.setFillColor(sf::Color(255, 255, 255, 255));
		cueshape.setPosition(cueloc);
		return cueshape;
	};
